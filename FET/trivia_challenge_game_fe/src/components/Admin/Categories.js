  ...
  const handleBack = () => {
    ...
    navigate("/admin");
      ...
      const handleDelete = async (id) => {
        ...
        const response =  await axios.post(' https://907fx2wvif.execute-api.us-east-1.amazonaws.com/Dev/deletecategory', dataVal);
      ...
      return (
        ...
        <div className="bg-white w-full sm:w-[480px] mx-auto text-center margin top mt-3 p-4 rounded-md shadow-sm border-[1px]">
            <button className="font-bold text-3xl mb-4  mx-auto text-center" style={{ color: "blue" }} onClick={() => handleRedirect()}>Create Category</button>
        </div>
        ...
        <table style={{ width: '100%' }}>
          ...
          <tbody>
            {allcategories.map((item) => (
              <tr key={item.Id}>
                <td style={{ textAlign: 'center', width: '45%', color: "blue" }}>{item.Id}</td>
                <td style={{ textAlign: 'center', width: '35%', color: "blue" }}>{item.Category}</td>
